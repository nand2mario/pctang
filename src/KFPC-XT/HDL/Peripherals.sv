...
module PERIPHERALS #(
    ...
    logic [1:0] kb_selftest;    // 0: normal, 1: wait in reset, 2: self test, 3: generating 0xAA
    logic [18:0] kb_reset_cnt = 500000;  // 10ms = 500000 cycles
    ...
    begin
        if (reset)
            ...
            kb_reset_cnt <= kb_reset_cnt == 0 ? 0 : kb_reset_cnt - 1;
            send_aa <= 0;

            case (kb_selftest)
                0: begin
                    kb_reset_cnt <= 500000; // keyboard clock lowered
                    if (~port_b_out[6])
                        kb_selftest <= 1;   // wait in reset
                end
                1: begin
                    if (kb_reset_cnt == 0 && port_b_out[6]) begin
                        kb_selftest <= 2;   // mock self test
                        kb_reset_cnt <= 500000;   // duration 10ms
                    end else if (port_b_out[6])
                        kb_selftest <= 0;   // back to normal
                end
                2: begin
                    if (kb_reset_cnt == 0)
                        kb_selftest <= 3;   // generate 0xAA
                end
                3: begin
                    send_aa <= 1'b1;
                    $display("keyboard self test done");
                    kb_selftest <= 0;   // back to normal
                end
                default: ;
            endcase
