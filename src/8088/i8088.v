...
module i8088
  (
    input               CORE_CLK, // 100Mhz for cycle accuracy

    input               CLK,      // 4.77Mhz ~ 9.54Mhz when cycle_accrate is 1
    input               RESET,

    input               READY,    // memory or I/O device ready
    input               INTR,
    input               NMI,

    output [19:0]       ad_out,   // address/data output
    output [7:0]    		dout,     // data output
    input  [7:0]        din,      // data input

    output              lock_n,   // indicates that other system bus masters are not to gain control 
                                  // of the system bus while LOCK is active (LOW).
    output              s6_3_mux, 
    output [2:0]        s2_s0_out, // status bits output
    output [2:0]        SEGMENT,  // segment output

    output              biu_done, // BIU has completed the EU request
    input               cycle_accrate,
